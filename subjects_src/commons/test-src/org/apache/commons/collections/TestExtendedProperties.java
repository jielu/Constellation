 ...
 * 
 * @version $Revision: 646780 $ $Date: 2008-04-10 13:48:07 0100 (Thu, 10 Apr
 *          2008) $
 * 
...
public class TestExtendedProperties extends TestCase {
	...
	public void testRetrieve() {
		...
		assertTrue("This returns array",
				(eprop.getVector("number") instanceof java.util.Vector));
		assertTrue("This returns array",
				(eprop.getList("number") instanceof java.util.List));

		/*
		 * now test dan's new fix where we get the first scalar when we access a
		 * vector/list valued property
		 */
		assertTrue("This returns scalar",
				(eprop.getString("number") instanceof String));
		...
		assertTrue("This returns vector",
				(eprop.getVector("prop.string") instanceof java.util.Vector));
		assertTrue("This returns list",
				(eprop.getList("prop.string") instanceof java.util.List));

		String prop2 = "hey\\, that's a test";
		eprop.remove("prop.string");
		eprop.setProperty("prop.string", prop2);
		assertTrue("This returns array",
				(eprop.getString("prop.string") instanceof java.lang.String));
		...
		assertTrue("Returns the full string", subEprop.getString("string")
				.equals(prop));
		assertTrue("This returns string for subset",
				(subEprop.getString("string") instanceof java.lang.String));
		assertTrue("This returns array for subset",
				(subEprop.getVector("string") instanceof java.util.Vector));
		assertTrue("This returns array for subset",
				(subEprop.getList("string") instanceof java.util.List));
	...
	public void testSaveAndLoad() {
		...
		try {
			/*
			 * initialize value: one=Hello\World two=Hello\,World
			 * three=Hello,World
			 */
			...
			assertEquals("Commas not interpreted properly", 2,
					ep1.getVector("three").size());
			assertEquals("Commas not interpreted properly", "Hello", ep1
					.getVector("three").get(0));
			assertEquals("Commas not interpreted properly", "World", ep1
					.getVector("three").get(1));

			assertEquals("Commas not interpreted properly", 2,
					ep1.getList("three").size());
			assertEquals("Commas not interpreted properly", "Hello", ep1
					.getList("three").get(0));
			assertEquals("Commas not interpreted properly", "World", ep1
					.getList("three").get(1));
	...
	public void testTrailingBackSlash() {
		...
		try {
			/*
			 * initialize using: one=ONE two=TWO \\ three=THREE
			 */
			String s1 = "one=ONE\ntwo=TWO \\\\\nthree=THREE";
			byte[] bytes = s1.getBytes();
			ByteArrayInputStream bais = new ByteArrayInputStream(bytes);
			ep1.load(bais);
			assertEquals("Trailing back-slashes not interpreted properly", 3,
					ep1.size());
			assertEquals("Back-slash not escaped properly", "TWO \\",
					ep1.getString("two"));
		} catch (IOException ioe) {
	...
	public void testMultipleSameKey1() throws Exception {
		...
		/*
		 * initialize using: one=a one=b,c
		 */
		String s1 = "one=a\none=b,c\n";
		byte[] bytes = s1.getBytes();
		ByteArrayInputStream bais = new ByteArrayInputStream(bytes);
	...
	public void testMultipleSameKey2() throws Exception {
		...
		/*
		 * initialize using: one=a,b one=c,d
		 */
		String s1 = "one=a,b\none=c,d\n";
		byte[] bytes = s1.getBytes();
		ByteArrayInputStream bais = new ByteArrayInputStream(bytes);
	...
	public void testMultipleSameKey3() throws Exception {
		...
		/*
		 * initialize using: one=a,b one=c
		 */
		String s1 = "one=a,b\none=c\n";
		byte[] bytes = s1.getBytes();
		ByteArrayInputStream bais = new ByteArrayInputStream(bytes);
	...
	public void testInheritDefaultProperties() {
		...
		ExtendedProperties extended = ExtendedProperties
...
//	public void testCollections271_1() {
...
//	}
//
//	public void testCollections271_2() {
//		ExtendedProperties props = new ExtendedProperties();
//		props.setProperty("test", "\\\\\\\\192.168.1.91\\\\test");
//		props.getProperty("test");
//		assertEquals("\\\\192.168.1.91\\test", props.getProperty("test"));
...
//					assertEquals(Locale.getDefault().toString() + ": " + j,
//							"value", map.get(data[j][1]));
//				}
//			}
//		} finally {
